...
class RecipeRateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if rate == '':
            raise CustomValidationError({"rate_detail": "값을 입력해주세요."})
        if data['rate'] < 0:
            raise CustomValidationError({"rate_detail_less": "평점의 범위를 벗어났습니다."})
        elif data['rate'] > 10:
            raise CustomValidationError({"rate_detail_greater": "평점의 범위를 벗어났습니다."})
        else:
            return data
